...
var _ = Describe("ProjectReference controller reconcilation", func() {
	var (
		...
		reconciler           *ProjectReferenceReconciler
...
`,
	...
	Context("When project reference CR does not exist", func() {
		...
		It("Returns without error", func() {
			_, err := reconciler.Reconcile(context.TODO(), reconcile.Request{NamespacedName: projectReferenceName})
	...
	Context("When project claim CR is not PendingProject", func() {
		...
		It("Gets requeued after 5 seconds", func() {
			result, err := reconciler.Reconcile(context.TODO(), reconcile.Request{NamespacedName: projectReferenceName})
			Expect(err).NotTo(HaveOccurred())
